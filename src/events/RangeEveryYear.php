...
class RangeEveryYear implements TemporalExpression
{
    ...
    public function __construct(
        ...
        if ($this->start_month < 1 || $this->start_month > 12) {
            throw new \InvalidArgumentException(sprintf('start_month: "%s"', $start_month));
        }

        if ($this->end_month < 1 || $this->end_month > 12) {
            throw new \InvalidArgumentException(sprintf('end_month: "%s"', $end_month));
        }

        if ($this->start_day < 1 || $this->start_day > 31) {
            throw new \InvalidArgumentException(sprintf('start_day: "%s"', $start_day));
        }

        if ($this->end_day < 1 || $this->end_day > 31) {
            throw new \InvalidArgumentException(sprintf('end_day: "%s"', $end_day));
        }
    }
